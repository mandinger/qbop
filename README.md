...
### Docker by OS
The container must run behind ProtonVPN (or a VPN container like `gluetun`) so NAT-PMP (`natpmpc`) can return the forwarded port. Below are quick starts per OS.

#### Windows (Docker Desktop, PowerShell)
- Ensure Docker Desktop is installed and WSL2 backend is enabled.
- If you already route traffic via a VPN container, start qbop with the sample compose:
    - `docker compose -f docker-compose/docker-compose.yml up -d`
- If you need a VPN container, use `gluetun` and attach qbop to it (see Synology example in `docker-compose/community/synology-docker-compose.yml` for the `network_mode: service:gluetun` pattern). On Windows you can adapt the same approach.
- UI: http://localhost:4567

#### Debian/Ubuntu or other Linux (Docker Engine)
- Install Docker Engine and Compose Plugin from your distro.
- Start with the sample compose:
    - `sudo docker compose -f docker-compose/docker-compose.yml up -d`
- If you need a VPN container, pair qbop with `gluetun` and run qbop with `network_mode: service:gluetun` (see `docker-compose/community/synology-docker-compose.yml`).
- UI: http://localhost:4567

## Build From Source (Docker)
Build with your local code instead of pulling the image. Two approaches:

### A) Simple build + run
- `version.yml` provides the version injected at build time.
...
Notes
- For realistic networking (VPN), prefer the compose method below with a VPN container.
- Rebuild after code changes: rerun `docker build ...` (or use `--no-cache`).

### B) docker compose that builds locally
Create a local override file (not committed) that replaces the remote image with a build of your working tree.
    ...
    qbop:
        build:
            ...
            args:
                VERSION: ${VERSION}
        image: qbop:dev
        # Optional local bind mounts for easier log/DB access
        volumes:
            - ./data:/opt/qbop/data
            - ./log:/opt/qbop/log
```
...
- Compose reads a `.env` file automatically. For local dev you can start by copying:
    - `cp .env.example .env` (or use the provided Windows equivalent) and set `OPN_SKIP=true`, `QBIT_SKIP=true` to avoid external dependencies.
- If running behind a VPN container (e.g., `gluetun`), adapt the Synology example under `docker-compose/community/` by using `network_mode: service:gluetun`.
